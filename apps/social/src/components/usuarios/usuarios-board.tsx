...
const AREAS: { id: Area; label: string }[] = [
  { id: "ambos", label: "Ambos paneles" },
  { id: "redes", label: "Solo Redes" },
  { id: "noticias", label: "Solo Noticias" },
];
...
export function UsuariosBoard({
  ...
  const [rol, setRol] = useState<Rol>("moderador");
  const [area, setArea] = useState<Area>("ambos");
  ...
  function guardar() {
    ...
    startTransition(async () => {
      try {
        if (editing) {
          await actualizarUsuario(editing.id, { nombre, rol, area });
        } else {
          await crearUsuario({ email, password, nombre, rol, area });
        }
        setOpen(false);
        router.refresh();
  ...
  return (
    <div className="w-full">
      <PageHeader
        title="Usuarios"
        subtitle="Quién puede entrar al panel. Crear, editar rol/área y eliminar accesos."
        action={
            ...
            return (
              <Card key={u.id}>
                <CardBody className="flex items-center gap-3 py-3">
                  ...
                  <Badge tone="neutral">{AREA_LABEL[u.area]}</Badge>
                  <button
                    type="button"
